...
class LocationForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if self.pk:
            if Location.objects.filter(city__exact=city_value, country__exact=country_value).exclude(id=self.pk).exists():
                self._errors['city'] = self.error_class(['Orasul si tara deja exista'])
        else:
            if Location.objects.filter(city__exact=city_value, country__exact=country_value).exists():
                self._errors['city'] = self.error_class(['Orasul si tara deja exista'])
